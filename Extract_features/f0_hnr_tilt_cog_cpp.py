...
def get_textgrid_path(wav_path, loc):
    wav_file = wav_path.split("/")[-1]
    textgrid_file = wav_file.replace(".wav", ".TextGrid")
    textgrid_path = "/".join(wav_path.split('/')[:-1]) + "/" + loc + "_aligned/" + textgrid_file
    return textgrid_path

            
def main():
    folders = load_files("corpus_mfa_1channel/*/")
    metadonnees = load_ages("metadonnees_ESLO2_ENT_ENTJEUN.csv")
    with open("ESLO2_acoustic_measures_second_part.csv", "w") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["fichier","locuteur","sexe", "age","classe_age", "phoneme", "left" ,"right", "duration","COG", "spectral_tilt", "f0", "harmonicity", "CPP"])
        for folder in ft_progress(folders[57:]) :
            print(f"\n Processing folder {folder}....")
            for loc in metadonnees :
                if loc["directory"] == folder.split("/")[-2].strip():
                    age = loc["age"]
                    classe_age = get_age_cat(age)
                    sexe = loc["sexe"]
                    loc_name = loc["directory"]
                    lst_files = glob.glob(f"{folder}/*.wav")
                    for file in ft_progress(lst_files):
                        textgrid_path = get_textgrid_path(file, loc_name)
                        if os.path.isfile(textgrid_path):
                            sound, textgrid = read_files(file, textgrid_path)
                            ...
                            for i, interval in enumerate(intervals_phones) :
                                ...
                                writer.writerow([basename,loc_name,sexe,age,classe_age,label, contexte_gauche, contexte_droit,duration, cog,spectral_tilt,f0,mean_hnr, cepstral_peak_prominence])
