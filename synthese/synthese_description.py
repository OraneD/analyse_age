...
# -*- coding: utf-8 -*-
"""
Created on Sat May  4 23:06:33 2024
...
def main():
    files = load_files("outputs_labise/*/*.wav")
    metadonnees = load_ages("../metadonnees_ESLO2_ENT_ENTJEUN.csv")
    with open("ESLO2_synthesis_acoustic_measures.csv", "w") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["fichier", "age","classe_age", "phoneme", "left" ,"right", "duration","COG", "spectral_tilt", "f0", "harmonicity", "CPP"])
        for file in ft_progress(files) :
            basename = get_basename(file)
            for loc in metadonnees :
                if loc["directory"].split("_")[-1] == basename.split(".")[0].split("_")[-1]:
                    print(loc["directory"].split("_")[-1], basename.split(".")[0].split("_")[-1])
                    age = loc["age"]
                    ...
                    if os.path.isfile(file.replace(".wav", ".TextGrid")):
                        ...
                        for i, interval in enumerate(intervals_phones) :
                            ...
                            writer.writerow([basename,age,classe_age,label, contexte_gauche, contexte_droit,duration, cog,spectral_tilt,f0,mean_hnr, cepstral_peak_prominence])
