...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 15 15:41:27 2024
...
def matrix_to_singlevector(df):
    ...
    for feature in opensmile_features :
        for i in range(0,len(df)):
            ...
            new_dataframe[new_col_name] = df[feature].iloc[i] if feature in df.columns else None       
...
def main():
    ...
    for i, row in enumerate(metadata) :
            ...
            if not os.path.exists(f"IS12_SingleVector_4sec/{sexe}/{age}/{folder}/"):
                os.mkdir(f"IS12_SingleVector_4sec/{sexe}/{age}/{folder}/")
            ...
            for file in ft_progress(file_lst) :
                    if get_time_sound(file) == 4 :
                        ...
                        spectral_df.to_csv(f"IS12_SingleVector_4sec/{sexe}/{age}/{folder}/{filename}_IS12_1vec.csv")
