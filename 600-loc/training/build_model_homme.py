...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 15:44:56 2024
...
X_train, y_train, all_filenames_train = load_train(metadata, "homme")
x_test, y_test, all_filenames_test = get_ESLO_vector("homme")
...
model.add(Dense(128, input_shape=input_shape, activation='relu',kernel_regularizer=l2(0.03)))
#model.add(Dense(64, input_shape=input_shape, activation='relu',kernel_regularizer=l2(0.09)))
model.add(Dropout(0.3))
#model.add(Dense(32, activation='relu', kernel_regularizer=l2(0.09)))
#model.add(Dense(16, activation='relu', kernel_regularizer=l2(0.09)))
model.add(Dense(3, input_shape=input_shape, activation='softmax'))


early_stopping = EarlyStopping(monitor='val_accuracy', patience=10, restore_best_weights=True)
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=2, min_lr=0.00001)
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
resume = model.fit(X_train, y_train_1_hot, epochs=100, batch_size=50, verbose=1, validation_data=(x_test,y_test_1_hot),callbacks=[reduce_lr], class_weight=class_weights_dict)
