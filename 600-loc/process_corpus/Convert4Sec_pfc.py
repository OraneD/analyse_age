...
def cut_every4sec(full_sound,loc, age_cat , sexe):
    ...
    while start <= duration_sound :
        ...
        sound_4sec.save(f"../PFC_4Sec/{sexe}/{age_cat}/{loc}/{loc}_4Sec_{num_file}.wav", "WAV")
        num_file += 1
        start += 4
        end += 4
            
def main():
    folder_lst = glob.glob("../../corpus_pfc/")
    print(folder_lst)
    for folder in ft_progress(folder_lst):
        ...
        sexe = folder.split("/")[-3]
        if not os.path.exists(f"../PFC_4Sec/{sexe}/{age_cat}/{loc}"):
            os.mkdir(f"../PFC_4Sec/{sexe}/{age_cat}/{loc}")
            path = f"{folder}/*.wav"
